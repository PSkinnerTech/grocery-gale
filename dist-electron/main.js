import require$$1$4, { app, ipcMain, BrowserWindow, shell } from "electron";
...
function update(win2) {
  ...
  main$1.autoUpdater.disableWebInstaller = false;
  main$1.autoUpdater.allowDowngrade = false;
  main$1.autoUpdater.on("checking-for-update", function() {
  });
  main$1.autoUpdater.on("update-available", (arg) => {
    win2.webContents.send("update-can-available", { update: true, version: app.getVersion(), newVersion: arg == null ? void 0 : arg.version });
  });
  main$1.autoUpdater.on("update-not-available", (arg) => {
  ...
  main$1.autoUpdater.on("download-progress", (progressObj) => {
    win2.webContents.send("download-progress", progressObj);
  });
  main$1.autoUpdater.on("update-downloaded", () => {
    win2.webContents.send("update-downloaded");
  });
...
ipcMain.handle("open-win", (_, arg) => {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    childWindow.loadURL(`${indexHtml}#${arg}`);
  }
});
